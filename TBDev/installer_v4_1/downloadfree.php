...
mysql_query("UPDATE freeslots SET free='yes', addedfree=$added WHERE torrentid=$id && userid=$CURUSER[id] && doubleup='yes'") or sqlerr(__FILE__, __LINE__);
elseif ($pq && $arrfs["doubleup"] == 'no')
mysql_query("INSERT INTO freeslots (torrentid, userid, free, addedfree) VALUES ($id,$userid,'yes',$added)") or sqlerr(__FILE__, __LINE__);
else
mysql_query("INSERT INTO freeslots (torrentid, userid, free, addedfree) VALUES ($id,$userid,'yes',$added)") or sqlerr(__FILE__, __LINE__);
// write_log("User id=<a href=$DEFAULTBASEURL/userdetails.php?id=$userid >$userid</a> [$CURUSER[username]] has used a freeleech slot on torrent id=<a href=$DEFAULTBASEURL/details.php?id=$id>$id</a>.");
///
...
stdhead();
print("<tr><td class=rowhead width=1%>Download</td><td width=99% align=left>Sorry, but you have no freeleech slots.<img src='/pic/smilies/sad.gif' border='0' alt=':('></td></tr>");
