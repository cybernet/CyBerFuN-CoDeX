...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	//send the pm !!
	sql_query("INSERT into messages (sender, receiver, added, msg) VALUES ".join(",",$pms)) or print(mysql_error());
	//update user download amount
	sql_query("INSERT INTO users (id,downloaded) VALUES ".join(",",$new_download)." ON DUPLICATE key UPDATE downloaded=values(downloaded)") or print(mysql_error());
	deletetorrent($tid);
	stderr(":w00t:","it worked! long live tbdev!");
	write_log("torrentdelete", "Torrent $tname was deleted by ".$CURUSER["username"]." and users Re-Paid Download");
