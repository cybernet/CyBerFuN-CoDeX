...
if (!isset($CURUSER) || ($CURUSER["id"] != $row["owner"] && get_user_class() < UC_MODERATOR)) {
    ...
    echo("<p>You're not the rightful owner, or you're not <a href=\"loginproc/index.php?returnto=" . urlencode(substr($_SERVER["REQUEST_URI"], 1)) . "&amp;nowarn=1\">logged in</a> properly.</p>\n");
} else {
...
if (file_exists($modfile) && filemtime($modfile) > (time() - $expire)) {
        ...
        fclose($modcache);
   echo '<h1><font size="+1"><font color="#FF0000">'.$ismoddin.'</font> is currently editing this torrent!</font></h1>';
   }
   else
   {
