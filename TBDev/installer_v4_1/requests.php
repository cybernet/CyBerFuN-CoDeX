...
if ($usergroups['canrequest'] == 'no' OR $usergroups['canrequest'] != 'yes') {
...
exit;
	...
	$search =isset($_GET["search"]) ? safeChar($_GET["search"]) : ""; //search string
	...
	elseif($do == "fill" && $rid > 0)
	{
		...
		else
		{
			...
			{
				$url = isset($_POST["url"]) ? safeChar($_POST["url"]) : "";
				if(empty($url))
				...
				$body = $CURUSER["username"]." filled this request ".safeChar($a["request"])."\n you can download the torrent from this url ".$url;
			...
			stdhead("Fill request ".safeChar($a["request"]));
				?>
				<form action="" method="post">
				<table width="400" cellpadding="3" cellspacing="2" style="border-collapse:separate;" align="center">
				  <tr>
					<td width="100%" colspan="2" align="left" class="colhead">Fill request : <?=safeChar($a["request"])?> added <?=(get_elapsed_time(sql_timestamp_to_unix_timestamp($a["added"])))?> ago </td></tr>
					<tr>
	...
	elseif($do == "add" || $do =="edit")
	{
		...
		if($_SERVER["REQUEST_METHOD"] == "POST")
		{
			$request = isset($_POST["request"]) ? safeChar($_POST["request"]) : "";
			if(empty($request))
				stderr("Err","Request name cant be empty!");
			$cat = isset($_POST["cat"]) ? 0+$_POST["cat"] : 0;
			if($cat == 0)
				stderr("Err","You have to select a category!");
			$descr = isset($_POST["descr"]) ?  safeChar($_POST["descr"]) : "";
			if(empty($descr))
				stderr("Err","You must give us some info about the request !");
			
			$message = "New Request : " . safeChar($request) . " requested by " . safeChar($CURUSER["username"]) . "";
		...
		<table width="400" cellpadding="3" cellspacing="2" style="border-collapse:separate;" align="center">
		  <tr>
			<td width="100%" colspan="3" align="center" class="colhead"><?=($do == "edit" ? "Edit request ".safeChar($a["request"])." made by <a href=\"userdetails.php?id=".$a["userid"]."\">".(!$a["username"] ? "unknown[".$a["userid"]."]" : $a["username"])."</a>" : "Add new request")?></td></tr>
			<tr>
		...
		else
		{
			...
			?>
				<table width="600" cellpadding="3" cellspacing="2" style="border-collapse:separate" align="center" >
				  <tr>
					<td colspan="3" class="colhead">Details for request : <?=safeChar($a["request"])?></td>
				  </tr>
				  <tr>
	...
	<table width="800" cellpadding="3" cellspacing="2" style="border-collapse:separate;">
	  ...
	  <tr>
	   <td align="center" colspan="3">You searched for <b><em><?=safeChar($search)?></em></b> <br/>if you didn't find what you wanted make a <input type="button" value="new request" onclick="window.location.href='<?=$this_url?>?do=add'"/></td>
	  </tr>
	  <?php } ?>
	</table>
	</form>
	...
	<table width="800" cellpadding="3" cellspacing="2" style="border-collapse:separate;">
	  <tr>
		<td align="left" width="70%"  rowspan="2"><a href="<?=$this_url."?do=details&amp;rid=".$a["id"]?>" ><?=safeChar($a["rname"])?></a><br/>
