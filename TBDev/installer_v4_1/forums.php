...
{
    global $pic_base_url, $forum_width, $DEFAULTBASEURL, $CURUSER ;
    ...
    while ($arr = mysql_fetch_assoc($res)) {
        ...
        "\" class='MagicZoom' title=\"" . safeChar($arr["username"]) . "\" class=\"borderimage\" onMouseover=\"borderit(this,'black')\" onMouseout=\"borderit(this,'silver')\"><img src=\"" .
    ...
    ?></td>
        ...
        <tr>
            <td class='colhead' align='center'><h2>Our members wrote <b><?php echo number_format($topic_post_arr['posts']);
    ?></b> Posts in <b><?php echo number_format($topic_post_arr['topics']);
    ?></b> Threads</h2></td>
        </tr>
	</table><?php
}
...
{
    global $CURUSER, $pic_base_url, $READPOST_EXPIRY, $DEFAULTBASEURL , $ss_uri ;
    ...
    while ($forums_arr = mysql_fetch_assoc($forums_res)) {
        ...
        if ($subforums == false && !empty($sfa[$forumid])) {
            if (($sfa[$forumid]['lastpost']['postid'] > $forums_arr['pid'])) {
                $lastpost = "" . $sfa[$forumid]['lastpost']['added'] . "<br />" . "by <a href='$DEFAULTBASEURL/userdetails.php?id=" . (int)$sfa[$forumid]['lastpost']['userid'] . "'><b>" . htmlspecialchars($sfa[$forumid]['lastpost']['user']) . "</b></a><br />" . "in <a href='" . $_SERVER['PHP_SELF'] . "?action=viewtopic&amp;topicid=" . (int)$sfa[$forumid]['lastpost']['topic'] . "&amp;page=p" . $sfa[$forumid]['lastpost']['postid'] . "#p" . $sfa[$forumid]['lastpost']['postid'] . "'><b>" . htmlspecialchars($sfa[$forumid]['lastpost']['tname']) . "</b></a>";
            } elseif (($sfa[$forumid]['lastpost']['postid'] < $forums_arr['pid'])) {
                $lastpost = "" . $forums_arr["added"] . "<br />" . "by <a href='$DEFAULTBASEURL/userdetails.php?id=" . (int)$forums_arr["userid"] . "'><b>" . htmlspecialchars($forums_arr['username']) . "</b></a><br />" . "in <a href='" . $_SERVER['PHP_SELF'] . "?action=viewtopic&amp;topicid=" . (int)$forums_arr["topicid"] . "&amp;page=p$lastpostid#p$lastpostid'><b>" . htmlspecialchars($forums_arr['subject']) . "</b></a>";
            } else
                $lastpost = "N/A";
        } else {
        ...
        ?>'><b><?php echo htmlspecialchars($forums_arr["name"]);
        ?></b></a><?php
        ...
        if ($show_mods == true && isset($mods_array[$forumid]))
            print("<br/>" . showMods($mods_array[$forumid]));
...
function insert_compose_frame($id, $newtopic = true, $quote = false, $attachment = false)
{
    ...
    if ($newtopic) {
        ...
        ?><h3 align="center">Reply to topic: <a href='<?php echo $_SERVER['PHP_SELF'];
        ?>action=viewtopic&amp;topicid=<?php echo $id;
        ?>'><?php echo htmlspecialchars($arr["subject"]);
        ?></a></h3><?php
    }
    ...
    if ($use_attachment_mod && $attachment) {
        ...
        ?><tr>
				<td colspan='2'><fieldset class="fieldset"><legend>Add Attachment</legend>
                    ...
                    <div class='error'>Allowed Files: rar, zip<br />Size Limit <?php echo prefixed($maxfilesize);
        ?></div></fieldset>
				</td>
			</tr><?php
    }
    ...
    ?>
		...
		<tr>
            ...
            <input type='submit' value='Submit' /><input type='button' class='button' value='Spell Check' onclick='openspell();'>
			</td>
		</tr>
...
if ($action == 'updatetopic') {
    ...
    if (isMod($topic_arr["forumid"]) || $CURUSER['class'] >= UC_MODERATOR) {
        ...
        if ((isset($_GET['delete']) ? $_GET['delete'] : (isset($_POST['delete']) ? $_POST['delete'] : '')) == 'yes') {
            ...
            write_log("topicdelete","Topic <b>" . $subject . "</b> was deleted by <a href='$DEFAULTBASEURL/userdetails.php?id=" . $CURUSER['id'] . "'>" . $CURUSER['username'] . "</a>.");
    ...
    // //////////////bot for topics
        ...
        if (!in_array($forumid, array("14","15","16","17","18","27","40"))) {
        autoshout($message);
        }
        ...
        sql_query("UPDATE users SET seedbonus = seedbonus+3.0, post_count = post_count + 1 WHERE id = " . sqlesc($CURUSER['id'])) or sqlerr(__FILE__, __LINE__);
        } else {
        ...
        if (mysql_num_rows($res) > 0) {
            ...
            if (($locked && $CURUSER['class'] < UC_MODERATOR) ? true : $voted) {
                ...
                for ($i = 0; $a = $os[$i]; ++$i) {
                    ...
                    ?>' nowrap="nowrap"><?php echo htmlspecialchars($a[1]);
                    ?></td><?php
    ...
    ?>?action=viewforum&amp;forumid=<?php echo $forumid;
    ?>"><?php echo $forum;
    ?></a> &gt; <?php echo htmlspecialchars($subject);
    ?></h1><?php
...
<?php
    ...
    while ($arr = mysql_fetch_assoc($res)) {
        ...
        if ($use_attachment_mod && ((!empty($arr['at_filename']) && is_valid_id($arr['at_id'])) && $arr['at_postid'] == $postid)) {
            ...
            $body .= "<div style=\"padding:6px\"><fieldset class=\"fieldset\">
					...
					<td nowrap=\"nowrap\"><a href=\"" . $_SERVER['PHP_SELF'] . "?action=attachment&amp;attachmentid=" . $arr['at_id'] . "\" target=\"_blank\">" . htmlspecialchars($arr['at_filename']) . "</a> (" . prefixed($arr['at_size']) . ", " . $arr['at_downloads'] . " downloads)</td>
        ...
        ?>" border="0" alt="PM <?php echo htmlspecialchars($postername);
        ?>" /></a>&nbsp;
				<a href='<?php echo $DEFAULTBASEURL;
    ...
    if ($CURUSER['class'] >= UC_MODERATOR || isMod($forumid)) {
        ...
        ?>
		<tr>
			<td colspan="2" class='colhead'>Staff options
		...
		<tr>
			<td class="rowhead" nowrap="nowrap">Delete topic</td>
			<td>
				<select name="delete">
    ...
    if ($maypost) {
        ...
        ?>" /><input type=submit value='Answer' />&nbsp;&nbsp;&nbsp;<input type='button' class='button' value='Spell Check' onclick='openspell();'></form>
		</td>
		<?php
    }
	...
	<tr>
		<td align=center colspan=2><input type=submit value='Update post' class=gobutton /><input type='button' class='button' value='Spell Check' onclick='openspell();'></td>
	</tr>
	</table>
    ...
    if ($CURUSER["class"] >= UC_MODERATOR || isMod($a["forumid"])) {
        ...
        if (!$sure)
            ...
            write_log("topicdelete","Topic <b>" . $a["subject"] . "</b> was deleted by <a href='$DEFAULTBASEURL/userdetails.php?id=" . $CURUSER['id'] . "'>" . $CURUSER['username'] . "</a>.");
    ...
    ?>
	<tr><td class=rowhead>Question <font color=red>*</font></td><td align=left><textarea name=question cols=70 rows=4><?php echo ($subaction == "edit" ? htmlspecialchars($poll['question']) : '');
    ?></textarea></td></tr>
	<tr><td class=rowhead>Option 1 <font color=red>*</font></td><td align=left><input name=option0 size=80 maxlength=40 value="<?php echo ($subaction == "edit" ? htmlspecialchars($poll['option0']) : '');
    ?>"><br/></td></tr>
	<tr><td class=rowhead>Option 2 <font color=red>*</font></td><td align=left><input name=option1 size=80 maxlength=40 value="<?php echo ($subaction == "edit" ? htmlspecialchars($poll['option1']) : '');
    ?>"><br/></td></tr>
	<tr><td class=rowhead>Option 3</td><td align=left><input name=option2 size=80 maxlength=40 value="<?php echo ($subaction == "edit" ? htmlspecialchars($poll['option2']) : '');
    ?>"><br/></td></tr>
	<tr><td class=rowhead>Option 4</td><td align=left><input name=option3 size=80 maxlength=40 value="<?php echo ($subaction == "edit" ? htmlspecialchars($poll['option3']) : '');
    ?>"><br/></td></tr>
	<tr><td class=rowhead>Option 5</td><td align=left><input name=option4 size=80 maxlength=40 value="<?php echo ($subaction == "edit" ? htmlspecialchars($poll['option4']) : '');
    ?>"><br/></td></tr>
	<tr><td class=rowhead>Option 6</td><td align=left><input name=option5 size=80 maxlength=40 value="<?php echo ($subaction == "edit" ? htmlspecialchars($poll['option5']) : '');
    ?>"><br/></td></tr>
	<tr><td class=rowhead>Option 7</td><td align=left><input name=option6 size=80 maxlength=40 value="<?php echo ($subaction == "edit" ? htmlspecialchars($poll['option6']) : '');
    ?>"><br/></td></tr>
	<tr><td class=rowhead>Option 8</td><td align=left><input name=option7 size=80 maxlength=40 value="<?php echo ($subaction == "edit" ? htmlspecialchars($poll['option7']) : '');
    ?>"><br/></td></tr>
	<tr><td class=rowhead>Option 9</td><td align=left><input name=option8 size=80 maxlength=40 value="<?php echo ($subaction == "edit" ? htmlspecialchars($poll['option8']) : '');
    ?>"><br/></td></tr>
	<tr><td class=rowhead>Option 10</td><td align=left><input name=option9 size=80 maxlength=40 value="<?php echo ($subaction == "edit" ? htmlspecialchars($poll['option9']) : '');
    ?>"><br/></td></tr>
	<tr><td class=rowhead>Option 11</td><td align=left><input name=option10 size=80 maxlength=40 value="<?php echo ($subaction == "edit" ? htmlspecialchars($poll['option10']) : '');
    ?>"><br/></td></tr>
	<tr><td class=rowhead>Option 12</td><td align=left><input name=option11 size=80 maxlength=40 value="<?php echo ($subaction == "edit" ? htmlspecialchars($poll['option11']) : '');
    ?>"><br/></td></tr>
	<tr><td class=rowhead>Option 13</td><td align=left><input name=option12 size=80 maxlength=40 value="<?php echo ($subaction == "edit" ? htmlspecialchars($poll['option12']) : '');
    ?>"><br/></td></tr>
	<tr><td class=rowhead>Option 14</td><td align=left><input name=option13 size=80 maxlength=40 value="<?php echo ($subaction == "edit" ? htmlspecialchars($poll['option13']) : '');
    ?>"><br/></td></tr>
	<tr><td class=rowhead>Option 15</td><td align=left><input name=option14 size=80 maxlength=40 value="<?php echo ($subaction == "edit" ? htmlspecialchars($poll['option14']) : '');
    ?>"><br/></td></tr>
	<tr><td class=rowhead>Option 16</td><td align=left><input name=option15 size=80 maxlength=40 value="<?php echo ($subaction == "edit" ? htmlspecialchars($poll['option15']) : '');
    ?>"><br/></td></tr>
	<tr><td class=rowhead>Option 17</td><td align=left><input name=option16 size=80 maxlength=40 value="<?php echo ($subaction == "edit" ? htmlspecialchars($poll['option16']) : '');
    ?>"><br/></td></tr>
	<tr><td class=rowhead>Option 18</td><td align=left><input name=option17 size=80 maxlength=40 value="<?php echo ($subaction == "edit" ? htmlspecialchars($poll['option17']) : '');
    ?>"><br/></td></tr>
	<tr><td class=rowhead>Option 19</td><td align=left><input name=option18 size=80 maxlength=40 value="<?php echo ($subaction == "edit" ? htmlspecialchars($poll['option18']) : '');
    ?>"><br/></td></tr>
	<tr><td class=rowhead>Option 20</td><td align=left><input name=option19 size=80 maxlength=40 value="<?php echo ($subaction == "edit" ? htmlspecialchars($poll['option19']) : '');
    ?>"><br/></td></tr>
    ...
    if (mysql_num_rows($res) == 0)
        ...
        $dls = 0;
        while ($arr = mysql_fetch_assoc($res)) {
            echo "<tr align='center'>" . "<td>" . htmlspecialchars($arr['filename']) . "</td>" . "<td><a class='pointer' onclick=\"opener.location=('/userdetails.php?id=" . (int)$arr['userid'] . "'); self.close();\">" . htmlspecialchars($arr['username']) . "</a></td>" . "<td>" . (int)$arr['downloads'] . "</td>" . "<td>" . $arr['date'] . " (" . get_elapsed_time(sql_timestamp_to_unix_timestamp($arr['date'])) . ")</td>" . "</tr>";
    ...
    stdhead("Forum - " . htmlspecialchars($arr["forum_name"]));
    begin_main_frame();
    if ($FORUMS_ONLINE == '0')
        stdmsg('Warning', 'Forums are currently in maintainance mode');
	...
	<table border=1 cellspacing=0 cellpadding=5 width='<?php echo $forum_width;
        ?>'>
		<tr><td colspan="4" class=colhead align=left><?php echo htmlspecialchars($arr["forum_name"]);
        ?> : SubForums</td></tr>
		<tr>
    ...
    if (mysql_num_rows($topics_res) > 0) {
        ...
        ?>>
		<tr><td colspan="7" class=colhead align=left><?php echo htmlspecialchars($arr["forum_name"]);
        ?> : Forums</td></tr>
		<tr>
			<td  align=left colspan="3">Topic</td>
			<td >Replies</td>
			<td >Views</td>
        ...
        while ($topic_arr = mysql_fetch_assoc($topics_res)) {
            ...
            ?>?action=viewtopic&amp;topicid=<?php echo $topicid;
            ?>'><?php echo htmlspecialchars($topic_arr['subject']);
        ...
        if ((isset($_POST[$action . "_action"]) ? $_POST[$action . "_action"] : '') == 'clear') {
			...
			<?php
				...
				<tr>
                    ...
                    ?>&amp;page=last#last'><?php echo htmlspecialchars($arr['subject']);
    ...
    <tr class="colhead" align="center">
        ...
        while ($getdaily = mysql_fetch_assoc($res)) {
            ...
            ?>#p<?php echo $postid ?>"><?php echo htmlspecialchars($getdaily['subject']);
            ?></a><br /><?php
            ?><b>In</b>&nbsp;<a href="<?php echo $_SERVER['PHP_SELF'];
            ?>?action=viewforum&amp;forumid=<?php echo (int)$getdaily['forumid'];
            ?>"><?php echo htmlspecialchars($getdaily['name']);
            ?></a><?php
            ?></td><?php
            ...
            if (!empty($getdaily['username'])) {
                ...
                ?>"><?php echo htmlspecialchars($getdaily['username']);
                ?></a><?php
            } else {
...
die;
	...
	<h1 align="center"><b><a href='<?php echo $_SERVER['PHP_SELF'];
        ?>'>Forums</a></b> -> <?php echo htmlspecialchars($arr["name"]);
        ?></h1>
	...
	<table border=1 cellspacing=0 cellpadding=5 width='<?php echo $forum_width;
			...
			<td align='left' class='colhead' width="100%">
                ...
                ?>'><b><font color='#3333FF'><?php echo htmlspecialchars($ovfname);
                ?></font></b></a>
			</td>
			...
			<td align='left' class='colhead'><font color='#3333FF'><b>Last post</b></font></td>
                ...
                show_forums($ovfid, false, $forums, $f_mod, true);
            }
            end_table();
            ...
            ?><p align='center'>
	<a href='<?php echo $_SERVER['PHP_SELF'];
            ?>?action=search'><b>Search Forums</b></a> |
	<a href='<?php echo $_SERVER['PHP_SELF'];
            ?>?action=viewunread'><b>New Posts</b></a> |
	<a href='<?php echo $_SERVER['PHP_SELF'];
            ?>?action=getdaily'><b>Todays Posts (Last 24 h.)</b></a> |
	<a href='<?php echo $_SERVER['PHP_SELF'];
            ?>?catchup'><b>Mark all as read</b></a><?php
