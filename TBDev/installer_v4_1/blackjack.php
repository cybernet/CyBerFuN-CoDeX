...
if (get_user_class() < UC_POWER_USER OR $CURUSER["blackjackban"] == 'yes') {
    stdhead();
    stdmsg("Sorry...", "You must be a Power User or above to play Blackjack... Or yer banned :).");
    stdfoot();
    exit;
}
    ...
    if ($_POST["game"] == 'start') {
        ...
        if ($points == 21) {
            ...
            if ($waitarr[0] > 0) {
                ...
                echo("<tr><td align=center>Your opponent was " . get_user_name($a["userid"]) . ", they had $a[points] points, $winorlose.<br /><br /><center><b><a href=$DEFAULTBASEURL/blackjack.php>Play again</a></b></center></td></tr>");
            } else {
                sql_query("update blackjack set status = 'waiting', date='" . get_date_time() . "' where userid = $CURUSER[id]") or sqlerr(__FILE__, __LINE__);
                echo("<tr><td align=center>There are no other players, so you'll have to wait until someone plays against you.<br />You will receive a PM with the game results.<br /><br /><center><b><a href=$DEFAULTBASEURL/blackjack.php>Back</a></b><br /></center></td></tr>");
            }
            echo("</table>");
            echo("</td></tr></table><br>");
            stdfoot();
