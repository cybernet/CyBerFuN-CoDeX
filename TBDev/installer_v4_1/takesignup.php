...
$ret = sql_query($q = ("INSERT INTO users (username, passhash, passhint, hintanswer, secret, editsecret, email, added, status" .
        (!$arr[0]?", class":"") . (!ENA_EMAIL_CONFIRM?", last_access, enabled":"") . ") VALUES (" .
        implode(",", array_map("sqlesc",
                array($wantusername, $wantpasshash, $passhint, $wanthintanswer, $secret, $editsecret, $email, get_date_time()))) . ",'" . (!$arr[0] || !ENA_EMAIL_CONFIRM?'confirmed':'pending') . "'" .
        (!$arr[0]?', ' . UC_SYSOP:'') .
...
if ($arr[0] || ENA_EMAIL_CONFIRM)
    ...
    sublogincookie($id, $wantpasshash);
