...
if (get_user_class() >= UC_ADMINISTRATOR)
    ...
    */
    ...
    if (isset($_POST['poster']) && (($poster = $_POST['poster']) != $fetch_assoc['poster'] && !empty($poster)))
        ...
        //===hidden and staff and count stats
        if ((isset($_POST['hidden'])) && (($hidden = $_POST['hidden']) != $row['hidden'])){    
        if ($CURUSER["hiddentorrents"] == "yes"){
        if ($row["hidden"] == "yes" && $hidden == "no"){
        $updateset[] = "added = '" . get_date_time() . "'";
        write_log("torrentupload", "Torrent $id (".sqlesc($_POST['name']).") was uploaded by " . $CURUSER["username"]);
        }
        $updateset[] = "hidden = " . sqlesc($hidden);
        }
        }
        //===staff
        if ((isset($_POST['staffonly'])) && (($staffonly = $_POST['staffonly']) != $row['staffonly'])){
        if(get_user_class() >= UC_MODERATOR){    
        if ($row["staffonly"] == "yes" && $staffonly == "no"){
        $updateset[] = "added = '" . get_date_time() . "'";
        write_log("torrentupload", "Torrent $id (".sqlesc($_POST['name']).") was uploaded by " . $CURUSER["username"]);
        }
        $updateset[] = "staffonly = " . sqlesc($staffonly);    
        }
        }
