...
$res = sql_query( "SELECT id, added, banned, vip, half, multiplicator, seeders + leechers AS numpeers, UNIX_TIMESTAMP(added) AS ts, countstats FROM torrents WHERE " . hash_where( "info_hash", $info_hash ) );
...
$res = sql_query( "SELECT $fields FROM peers WHERE torrent = $torrentid AND connectable = 'yes' $wantseeds $limit" ) or
...
// /////////////////////////// End compact mode////////////////////////////////
if ( !isset( $self ) ) {
    $res = sql_query( "SELECT $fields FROM peers WHERE $selfwhere" );
...
// === stop banned IPs
$nip = ip2long( $ip );
$res = sql_query( "SELECT id FROM bans WHERE $nip >= first AND $nip <= last" );
...
if ( !isset( $self ) ) {
    ...
    // ///// Torrent-Limit
    if ( $az["tlimitall"] >= 0 ) {
        $arr = mysql_fetch_assoc( sql_query( "SELECT COUNT(*) AS cnt FROM peers WHERE userid=$userid" ) );
        $numtorrents = $arr["cnt"];
        $arr = mysql_fetch_assoc( sql_query( "SELECT COUNT(*) AS cnt FROM peers WHERE userid=$userid AND seeder='yes'" ) );
        $seeds = $arr["cnt"];
    ...
    $rst = sql_query( "SELECT class, username, highspeed, uploaded, webseeder FROM users WHERE id = $userid " ) or
    err( "Ratio faker query failure" );
    ...
    $resfs = sql_query("SELECT * FROM freeslots WHERE torrentid=$torrentid && userid=$userid");
    ...
    elseif ( $multiplicator == "5" )
        $upthis = $upthis * 5;
    ///////////////////happyhour by putyn
    $happy = sql_query( "SELECT id, multiplier from happyhour where userid=" . sqlesc( $userid ) . " AND torrentid=" . sqlesc( $torrentid ) . " " );
    ...
    $q = sql_query("SELECT * FROM events ORDER BY startTime DESC LIMIT 1") or print (mysql_error());
	  ...
	  //==ratio faker ??
    $arr = mysql_fetch_assoc(sql_query("SELECT UNIX_TIMESTAMP(last_action) AS lastaction FROM peers WHERE $selfwhere"));
    $interval = time() - $arr["lastaction"];
    if ($interval == 0) $interval = 1;
    // If speed is higher than 4 MB/sec, the user is apparently cheating...
    // Frustrate him by adding his desired upload to his download ^00^ lol
    if (($upthis / $interval) > 4194304 ) {
    // Write log
    write_log("ratiocheat"," Announce.php: Ratiofaker-Tool used - Event 1: ".prefixed($upthis)." Upload, Fake Rate: ".prefixed($upthis / $interval)."/sec");
    //Add upload as download and zero out upload for event.
    $downthis += $upthis;
    $upthis = 0;
    }
        ...
        $udq = implode(',', $updq);
        sql_query("UPDATE users SET $udq WHERE id=$userid") or err("Tracker error 3");
        }
        // ///// Initial sanity check 1.5 MB/s for 1 second
        if ( $upthis > 1572864 ) {
            ...
            $rate = ( $upthis / ( $diff + 1 ) );
            // Currently 1.5 MB/s. Increase to 5MB/s once finished testing.
            if ( $rate > 1572864 ) {
...
//=== hit and run    
$res_snatch = sql_query("SELECT seedtime, uploaded, downloaded, finished, UNIX_TIMESTAMP(start_date) AS start_snatch FROM snatched WHERE torrentid = $torrentid AND userid = $userid") or err('Snatch Error 1');
...
// or using ratio in the equitation
...
// or using ratio in the equalization
...
sql_query("UPDATE snatched SET ip = ".sqlesc($ip).", port = $port, connectable = '$connectable', uploaded = uploaded + $upthis, downloaded = downloaded + $downthis, to_go = $left, upspeed = $upspeed, downspeed = $downspeed, $announcetime, last_action = '".get_date_time()."', seeder = 'no', agent = ".sqlesc($agent)." $hit_and_run WHERE torrentid = $torrentid AND userid = $userid") or err('SL Err 1');
    ...
    $updateset[] = ($self['seeder'] == 'yes' ? 'seeders = seeders - 1' : 'leechers = leechers - 1');
        ...
        sql_query( "UPDATE peers SET ip = " . sqlesc( $ip ) . ", port = $port, connectable = '$connectable', webseeder='$webseeder', uploaded = $uploaded, downloaded = $downloaded, to_go = $left, last_action = NOW(), prev_action = $prev_action, seeder = '$seeder', agent = " .
        ...
        if ( mysql_affected_rows() ) {
            ...
            sql_query( "UPDATE snatched SET ip = " . sqlesc( $ip ) . ", port = $port, connectable = '$connectable', uploaded = uploaded + $upthis, downloaded = downloaded + $downthis, to_go = $left, upspeed = $upspeed, downspeed = $downspeed, $announcetime, last_action = '" .
        ...
        sql_query( "INSERT INTO peers (torrent, userid, peer_id, ip, port, webseeder, connectable, uploaded, downloaded, to_go, started, last_action, seeder, agent, downloadoffset, uploadoffset, passkey) VALUES ($torrentid, $userid, " .
        ...
        if ( mysql_affected_rows() ) {
            $updateset[] = ( $seeder == "yes" ? "seeders = seeders + 1" :
                "leechers = leechers + 1" );
            $anntime = "timesann = timesann + 1";
            sql_query( "UPDATE snatched SET ip = " . sqlesc( $ip ) . ", port = $port, connectable = '$connectable', to_go = $left, last_action = '" .
