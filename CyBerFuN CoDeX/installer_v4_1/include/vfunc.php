...
foreach ($_GET as $check_url) {
	if (!is_array($check_url)) {
		...
		if ((eregi("<[^>]*script*\"?[^>]*>", $check_url)) || (eregi("<[^>]*object*\"?[^>]*>", $check_url)) ||
			...
			(eregi("\"", $check_url))) {
      stderr('Error', 'Time shall unfold what plighted cunning hides Who cover faults, at last shame them derides - Sorry Xss exploits dont work here punk.');
		}
	}
}
*/
//begin staff secure ip function//
...
function failedloginscheck ()
{
    ...
    if ($total >= $maxloginattempts) {
        ...
        stderr("Login Locked!", "You have been <b>exceed maximum login attempts</b>, therefore your ip address <b>(" . safeChar($ip) . ")</b> has been banned.");
    }
}
