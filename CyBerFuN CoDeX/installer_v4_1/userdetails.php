...
if (($id != $CURUSER['id']) AND ($usergroups['canviewotherprofile'] == 'no' OR $usergroups['canviewotherprofile'] != 'yes')) {
stderr( "Sorry...", "You dont have permission to view profiles" );
exit;
}

$r = sql_query("SELECT userid, port, agent FROM peers WHERE userid=".sqlesc($id)." LIMIT 1") or sqlerr(__FILE__, __LINE__);
if (mysql_num_rows($r) > 0)
{
while ($arr = mysql_fetch_assoc($r))
{
$agent = sqlesc($arr[agent]);
mysql_query("UPDATE users SET port=$arr[port], agent=$agent WHERE id=$arr[userid]") or sqlerr(__FILE__, __LINE__);
}
}
...
if ($user["anonymous"] == 'yes' && $CURUSER['class'] < UC_VIP) {
    ...
    if ($user["avatar"])
        echo ("<tr><td class=rowhead>".$language['avatar']."</td><td align=left><a href=\"" . safeChar($user["avatar"]) .
            "\" class='MagicZoom' title=\"" . safeChar($user["username"]) . "\" class=\"borderimage\" onMouseover=\"borderit(this,'black')\" onMouseout=\"borderit(this,'silver')\"><img src=\"" .
...
?>
<tr><td class="rowhead" valign="top" align="right" width='1%'>Reputation</td><td align='left' width='99%'><?php echo $member_reputation?><br />
</td></tr>
<?php
...
////////////////new time ratio system-sir snuggles/////////////////////////////////
//=== testing concept of "share ratio"
$res_share_ratio = sql_query("SELECT SUM(seedtime) AS seed_time_total, COUNT(id) AS total_number FROM snatched WHERE seedtime > '0' AND userid =".$user['id'])or sqlerr(__FILE__, __LINE__);
...
if (get_user_class() >= UC_MODERATOR && $user["class"] < get_user_class()) {
    ...
    echo("<tr><td class='heading' valign='top' align='right'>".$language['title']."</td><td colspan='2' align='left'><input type='text' size='60' name='title' value=\"" . (isset($user['pre_title'])?htmlspecialchars($user['pre_title']):$user['title']) . "\" /></td></tr>\n");
...
<input type=radio name=warns value=" . ($user["warns"] + 10) . ">" . ($user["warns"] +
         ...
         */
        echo("<tr><td class=rowhead>Enabled</td><td colspan=2 align=left><input name=enabled value='yes' type=radio" . ($user["enabled"]=="yes" ? " checked" : "") . ">Yes <input name=enabled value='no' type=radio" . ($user["enabled"]=="no" ? " checked" : "") . ">No</td></tr>\n");
        echo("<tr><td class=rowhead>Disable Reason</td><td colspan=2 align=left><input type=text size=70 name=reasond " . ($user["enabled"]=="no" ? " readonly value=".$user["disable_reason"]."" : "") . " ></td></tr>\n");
    } else {
    ...
    echo ("<tr><td class=rowhead>".$language['bban']."</td><td colspan=2 align=left><input type=radio name=blackjackban value=yes" .
        ...
        "no" ? " checked" : "") . ">No</td></tr>\n");
    echo("<tr><td class=rowhead>Lottery ban</td><td colspan=2 align=left><input type=radio name=lotteryban value=yes" .($user["lotteryban"] === "yes" ? " checked" : "").">Yes <input type=radio name=lotteryban value=no" .($user["lotteryban"] === "no" ? " checked" : "").">No</td></tr>\n");
    ...
    echo ("<tr><td class=rowhead>".$language['anon']."</td><td colspan=2 align=left><input type=radio name=anonymous value=yes" .
        ...
        " checked" : "") . ">No</td></tr>\n");
    //echo("<tr><td class=tableb>Bad User <img src=\"pic/help.png\" style=\"vertical-align:middle;\" title=\"Provided that this user is only invalid peer IPs\" alt=\"\"></td><td class=tablea align=left><input name=baduser value='yes' type=radio" . ($baduser ? " checked" : "") . ">Yes <input name=baduser value='no' type=radio" . (!$baduser ? " checked" : "") . ">No</td><td class=tablea align=left><a href=\"startstoplog.php?op=acclist&amp;id=$id\">List Accounts</a></td></tr>\n");
      /*
      echo ("<tr><td class=rowhead>Bad User <img src=\"pic/help.png\" style=\"vertical-align:middle;\" title=\"Provided that this user is only invalid peer IPs\" alt=\"\"></td><td colspan=2 align=left><input type=radio name=baduser value=yes" .
        ($user["baduser"] == 1 ? " checked" : "") .
        ">Yes <input type=radio name=baduser value=no" . ($user["baduser"] == 0 ?
        " checked" : "") . ">No<br><td class=tablea align=left><a href=\"startstoplog.php?op=acclist&amp;id=$id\">List Accounts</a></td></tr>\n");
    */
