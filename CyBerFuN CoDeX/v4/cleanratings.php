...
stdhead("Cleanup ratings");
begin_main_frame();
begin_frame("Cleaned ratings", false);

$sres = mysql_query("SELECT DISTINCT torrent FROM ratings") or sqlerr(__FILE__, __LINE__);
while ($sarr = mysql_fetch_assoc($sres)) {
    ...
    if (mysql_num_rows($ures) == 0)
        mysql_query("DELETE FROM ratings WHERE torrent = $sarr[torrent]") or sqlerr(__FILE__, __LINE__);
    @mysql_free_result($ures);
}
@mysql_free_result($sres);
write_log("rating_table_cleaned", "Rating Table Cleaned by " . $CURUSER["username"]);
