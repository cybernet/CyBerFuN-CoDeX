...
if ((isset($_POST['action'])) && ($_POST['action'] == "edituser")) {
    ...
    $curhiddentorrents = $user["hiddentorrents"];
...
PS. Your donator status will last for an extra $dur on top of your current donation status, and can be found on your user details page and can only be seen by you :smile: It was set by " .
    ...
    */
    ...
    if ((isset($_POST['title'])) && (($title = $_POST['title']) != ($curtitle = $user['title']))) {
        $modcomment = gmdate("Y-m-d") . " - Custom Title changed to '" . $title .
            "' from '" . $curtitle . "' by " . $CURUSER['username'] . ".\n" . $modcomment;
    ...
    //=== hiddentorrents
    if ((isset($_POST['hiddentorrents'])) && (($hiddentorrents = $_POST['hiddentorrents']) != $user['hiddentorrents'])){
    if ($hiddentorrents == 'yes')
    $modcomment = gmdate("Y-m-d") . " - Enabled Hidden Torrents - " . $CURUSER['username'] . ".\n" . $modcomment;
    else
    $modcomment = gmdate("Y-m-d") . " - Disabled Hidden Torrents - " . $CURUSER['username'] . ".\n" . $modcomment;
    $updateset[] = "hiddentorrents = " . sqlesc($hiddentorrents);
    }
    // FLS Support
	...
	if(isset($_POST["forum_mod"]) && ($forum_mod = $_POST["forum_mod"]) != $user["forum_mod"])
	{
		...
		$modcomment = gmdate("Y-m-d") ." ".$CURUSER["username"]." ".$whatm." forum rights\n" . $modcomment;

	}
	$forums = isset($_POST["forums"]) ? $_POST["forums"] : "";
	if(!empty($forums) && sizeof($forums))
	{
		$foo = "[".join("][",$forums)."]";
...
//end forum moderator mod 
    ...
    */
    ...
    // --------------------------------
    // enable hidden torrents
    // --------------------------------
    if ($hiddentorrents && $curhiddentorrents != $hiddentorrents) {
        if ($hiddentorrents == 'yes')
            write_info("User account $userid (<a href=userdetails.php?id=$userid>$user[username]</a>) hidden torrents enabled by <a href=userdetails.php?id=$CURUSER[id]>$CURUSER[username]</a>");
    }
    // --------------------------------
    // disable hidden torrents
    // --------------------------------
    if ($hiddentorrents && $curhiddentorrents != $hiddentorrents) {
        if ($hiddentorrents == 'no')
            write_info("User account $userid (<a href=userdetails.php?id=$userid>$user[username]</a>) hidden torrents disabled by <a href=userdetails.php?id=$CURUSER[id]>$CURUSER[username]</a>");
    }
    // --------------------------------
