...
if ($action == "add") {
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        sql_query("INSERT INTO comments (user, torrent, added, text, ori_text, anonymous, hidden, staff) VALUES (" . $CURUSER["id"] . ",$torrentid, '" . get_date_time() . "', " . sqlesc($text) . "," . sqlesc($text) . ", $anon, '1','0')");
    ...
    print("<p><form name=comment method=\"post\" action=\"hiddencomment.php?action=add\">\n");
    ...
    $res = sql_query("SELECT comments.id, comments.text, comments.added, comments.anonymous, username, users.id as user, users.avatar FROM comments LEFT JOIN users ON comments.user = users.id WHERE hidden = $torrentid ORDER BY comments.id DESC LIMIT 5");
    ...
    stdhead("Edit comment to hidden torrent \"" . safechar($arr["name"]) . "\"");
    begin_table();
    print("<h1>Edit comment to hidden torrent \"" . safechar($arr["name"]) . "\"</h1><p>\n");
    print("<form method=\"post\" name=\"compose\"action=\"hiddencomment.php?action=edit&cid=$commentid\">\n");
