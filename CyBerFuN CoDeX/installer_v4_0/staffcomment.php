...
if ($action == "add") {
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        sql_query("INSERT INTO comments (user, torrent, added, text, ori_text, anonymous, hidden, staff) VALUES (" . $CURUSER["id"] . ",$torrentid, '" . get_date_time() . "', " . sqlesc($text) . "," . sqlesc($text) . ", $anon, '0', '1')");
    ...
    stdhead("Add a staff comment to \"" . $arr["name"] . "\"");

    print("<h1>Add a staff comment to \"" . safechar($arr["name"]) . "\"</h1>\n");
    print("<p><form name=comment method=\"post\" action=\"staffcomment.php?action=add\">\n");
    ...
    $res = sql_query("SELECT comments.id, comments.text, comments.added, comments.anonymous, username, users.id as user, users.avatar FROM comments LEFT JOIN users ON comments.user = users.id WHERE staff = $torrentid ORDER BY comments.id DESC LIMIT 5");
    ...
    print("<form method=\"post\" name=\"compose\"action=\"staffcomment.php?action=edit&cid=$commentid\">\n");
